...
export const getProcessSummary = (scenario: AiProcessScenario, status: AiStreamStatus) => {
  if (scenario === 'polish') {
    ...
    if (status === 'background_running') return '连接已断开，当前润色已中止'
    if (status === 'resume_available') return '当前润色结果未完成'
  ...
  if (scenario === 'edit') {
    ...
    if (status === 'background_running') return '连接已断开，等待恢复当前修改任务'
    if (status === 'resume_available') return '当前修改任务可恢复'
  ...
  if (status === 'background_running') return '连接已断开，等待恢复当前生成任务'
  if (status === 'resume_available') return '当前生成任务可恢复'
